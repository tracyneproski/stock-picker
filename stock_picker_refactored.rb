...
def stock_picker(prices)
  pairs = []
  # maybe rename x and y as index_x and index_y to be clear
  y = prices.count - 1
  while y != 0
    x = y - 1
  ...
  # same logic written using map
  difference = pairs.map { |pair| pair[1] - pair[0] }
  # difference = []
  # pairs.each do |pair|
  ...
  # same logic without a varaible or push operator
  [prices.index(best_days[0]), prices.index(best_days[1])]
end
